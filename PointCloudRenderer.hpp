...
//    void push_back(float x, float y, float z, int r, int g, int b);
    ...
    // Added by soonmin, 151119
    void push_back(GlVec3 point, GlCol3 color, GlVec3 pt2d, uint flag = 0, float car = 0.0f, float ped = 0.0f );
    void push_back(float x, float y, float z, int r, int g, int b, float u, float v, float d, uint flag = 0, float car = 0.0f, float ped = 0.0f);

    GlVec3&     rangeAt(int index); //!< use to get or set point coordinate
    uint&       bInsideAt(int index);
    float&      carScAt(int index);
    float&      pedScAt(int index);
//    bool    isInside(int index);
//    float   getCarSc(int index);
//    float   getPedSc(int index);
...
private:
    ...
    // Added by soonmin, 151119
    std::vector<GlVec3>   range2d; // continuous memory-buffer for points to draw with OpenGL
    std::vector<uint>     bInside;
    std::vector<float>    scCar;
    std::vector<float>    scPed;
};
